...
import vtkMouseCameraSliceManipulator from './SliceManipulator';
import WindowLevelMouseManipulator from './WindowLevelMouseManipulator';
...
function vtkInteractorStyleMPR(publicAPI, model) {
  ...
  model.slicelManipulator = vtkMouseCameraSliceManipulator.newInstance(
    {
      button: 1,
    }
  );
  model.windowLevelManipulator = WindowLevelMouseManipulator.newInstance(
    {
      button: 1,
    }
  );
  ...
  function setManipulators() {
    ...
    //publicAPI.addMouseManipulator(model.windowLevelManipulator);
    publicAPI.addMouseManipulator(model.slicelManipulator);
  ...
  publicAPI.setVolumeMapper = (mapper) => {
    if (superSetVolumeMapper(mapper)) {
      ...
      if (mapper) {
        ...
        let viewUp = [0,1,0];
        publicAPI.setSliceNormal(publicAPI.getSliceNormal(),viewUp);
      } else {
  ...
  // in world space
  publicAPI.setSliceNormal = (normal,viewUp) => {
    const renderer = model.interactor.getCurrentRenderer();
    const camera = renderer.getActiveCamera();
