...
import vtkInteractorStyleTrackBall from 'vtk.js/Sources/Interaction/Style/InteractorStyleTrackballCamera';
...
function vtkImageRenderWindow(publicAPI, model) {
  ...
  const iStyle = vtkInteractorStyleTrackBall.newInstance();
  // iStyle.setInteractionMode('IMAGE_SLICING');
  model.interactor.setInteractorStyle(iStyle);
  ...
  publicAPI.resize = () => {
    if (model.container) {
      ...
      model.openGLRenderWindow.setSize(
        Math.floor(dims.width -1),
        Math.floor(dims.height -1)
      );
      invokeResize();
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  vtkImageRenderWindow(publicAPI, model);
}
